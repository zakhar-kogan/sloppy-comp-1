...
Date: 2026-04-23 (updated 2026-05-12)

This benchmark compares local runs of Codex, OMP, OpenCode, Claude Code, and Droid on the same TypeScript CLI task.
...
|---|---|---|
| GPT-5.4 | Codex, OMP, OpenCode, Claude Code, Droid | Five-way comparison completed. |
| Opus 4.6 | Codex, OMP, OpenCode, Claude Code, Droid | Five-way comparison completed. |
...
| Droid | BYOK custom model | `kr/claude-opus-4.6` via omniroute |
...
| Droid | 336.5 MB | 276.1 MB | 24.8% | 6.5% | ~8.3 min | ~68K | 760K | 827K | — |
...
| Droid | 286 | 8 embedded | No | No | No | No | Missing pagination; tests embedded in production CLI file. |
...
| 5 | Droid | Slowest GPT run; missing pagination and embedded tests affect CLI behavior. |
...
| Droid | 249.5 MB | 218.0 MB | 35.9% | 2.1% | ~1.3 min | ~2.6K reported | 0 | ~2.6K reported | 1 constrained |
...
| Droid | 215 | 6 embedded behind `--test` | Yes | No | No | Yes | Yes | No timeout/API version header; BYOK required one-Execute-tool workaround. |
...
| 2 | OMP | Lowest reliable token count, pagination, timeout, separate tests; higher RSS than Codex. |
| 3 | Droid | Fastest wall time and full pagination/clean JSON, but no timeout/API version header and no separate test file. |
| 4 | OpenCode | Much improved vs GPT run: paginated, clean JSON, separate tests; higher tokens/resources. |
| 5 | Claude Code | Pagination present, but output shape is structurally wrong because tests live inside the CLI file. |
...
| Droid | GPT-5.4 | 336.5 MB | 276.1 MB | 24.8% | 6.5% | ~8.3 min | ~68K | 760K | 827K | — | Slow, missing pagination, tests embedded in CLI file. |
| Droid | Opus 4.6 | 249.5 MB | 218.0 MB | 35.9% | 2.1% | ~1.3 min | ~2.6K reported | 0 | ~2.6K reported | 1 constrained | Fastest wall time; paginated and clean JSON, but no timeout/API version header. |
...
| Droid | 2 | 293.0 MB | 247.1 MB | 30.4% | 4.3% | ~4.8 min |
...
- Droid has moderate memory/CPU on average; the Opus run was fast but used a constrained one-tool workaround due BYOK/tool-call compatibility.
...
Note: Droid / Opus 4.6 BYOK accounting reported only input tokens and likely under-counts output/tool-call tokens.
...
| Droid | ~68K -> ~2.6K reported | Weak -> Solid but constrained | Much faster on Opus, with lower RSS and CPU; token accounting is incomplete |
...
| Droid / GPT-5.4 | No | No | No | No | Yes | Missing pagination; embedded tests run in CLI |
| Droid / Opus 4.6 | Yes | No | Yes | No | Yes | No timeout/API version header; one-tool workaround |
...
| Droid | Missing pagination and timeout. Tests are embedded in the CLI file and register during normal execution. JSON uses raw GitHub-style `stargazers_count` rather than a task-oriented `stars` field. |
...
| Droid | No material runtime defect in the generated CLI. Medium concern: no timeout and no API version header. Tests are embedded behind `--test` rather than separated. The benchmark required a one-Execute-tool workaround because the BYOK proxy accepted a simple prompt but failed multi-tool calls. |

## Final Ranking

| Scope | 1 | 2 | 3 | 4 | 5 |
|---|---|---|---|---|---|
| GPT-5.4 | Codex | OMP | Claude Code | OpenCode | Droid |
| Opus 4.6 | Codex | OMP | Droid | OpenCode | Claude Code |
| Across completed runs | Codex | OMP | OpenCode | Droid | Claude Code |
...
- Codex and Droid Opus 4.6 were routed via omniroute (`kr/claude-opus-4.6`); other tools used `quotio/kiro-claude-opus-4-6` or tool-native routing. These resolve to the same underlying model family.
- Droid / Opus 4.6 used a constrained one-Execute-tool workaround because the BYOK proxy failed after normal multi-tool calls with an "Improperly formed request" error.
...
- `results/droid-gpt54.csv`
- `results/droid-opus46.csv`
